<template>
  <div class="movie-info" v-if="detailData.id">
    <h1 class="title">{{ detailData.title }} {{ detailData.original_title }}<span class="year">（{{ detailData.year
    }}）</span>
    </h1>
    <div class="desc-content">
      <img class="mv-img" :src="detailData.cover_url" />
      <div class="mv-desc">
        <p>导演: <a href="#" v-if="detailData.directors && detailData.directors.length">
            {{ detailData.directors[0].name }}</a></p>
        <p>主演: <a href="#" v-for="(item, index) in actors.short" :key="index">{{ item.name }} <template
              v-if="index != actors.short.length - 1">/</template> </a><span @click="expand" class="more-actors"
            v-if="actors.isShowMore">更多..</span></p>
      ...
      <div class="mv-rank">
        ...
        <div class="better-content">
          <div v-for="(item, index) in rate.betterList" class="better-item">好于 <a href="#">{{ (item.rank * 100).toFixed(1)
            + '%' }} {{ item.type }}</a></div>
        </div>
      </div>
    </div>
...
onMounted(async () => {
  ...
  store.setTitle(detailData.value.title)
...
function goPublish() {
  if (store.userInfo.nickname) {
    router.push('/publish?id=' + detailData.id)
  } else {
    router.push('/login')
  }
}
