  ...
  <div class="publish-container">
    ...
    <div class="movie-info" v-if="movieData.title">
      ...
      <div class="right-info">
        ...
        <div class="desc">{{ movieData.card_subtitle }} {{ movieData.rating.value }}分({{ movieData.rating.count }}评价)
        </div>
      </div>
    </div>
    <div class="score-add">
...
function submit() {
  let count = 0
  starlist.list.forEach((item) => {
    if (item.state == 'full') {
      count++
    }
  })
  store.setCommentList({
    rating: {
      value: count
    },
    user: {
  ...
  router.push('/detail?id=' + movieData.id)
}
